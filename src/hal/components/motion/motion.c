...
static struct AXIS get_position_control_periods(struct AXIS x){
    ...
    if(dist>0){ //! Positive move.
        ...
        /*
            1.Calculate if vm can be reached :
                t1 from vo to vm, t3 from vm to ve,
                    if total s1+s3>dist, 2 periods to sample max velocity during movement.
                    if s1+s2<dist, 3 periods.

            2.Sample 2 periods t1,t3 :
                for t=0, to infinity, t+=time_step.
                s1= ..
                s2= ..
                s3= .. steady to fit curve.
                if(s1+s2>dist){ break.}
        */

        t1=(-vo+vm)/am;
        s1=0.5*(vo+vm)*t1;
        t2=(-vo+ve)/am; if(t2<0){t2*=-1;}
        s2=0.5*(vo+ve)*t2;
        if(s1+s2<dist){ //! 2 periods to sample.
            for(float i=vm; i>0; i-=0.001){
                t1=(-vo+i)/am;
                s1=0.5*(vo+i)*t1;
                t2=(-vo+ve)/am; if(t2<0){t2*=-1;}
                s2=0.5*(vo+ve)*t2;
